...
uint8_t getFingerprintEnroll() {
  ...
  Serial.print("ID "); Serial.println(id);
  p = finger.storeModel(id); //Ask the sensor to store the calculated model for later matching
  if (p == FINGERPRINT_OK) {
...
bool enroll_Driver()                    
{
  ...
  id = 101;
  Serial.print("Enrolling Driver at ID #");
  Serial.println(id);
  uint8_t val=0;
  while (!val){
    val = getFingerprintEnroll();
    };
  //get the driver's enrolled fingerprint
  
  if(val== true){
    //get driver's stored fingerprint
      char tmp[16];
      char format[128];
