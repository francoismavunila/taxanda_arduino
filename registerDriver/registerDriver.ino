...
const char* driversub_topic= "driver/register/getFP";

const char* driverpub_topic="drive/register/sendFP";

const char* driversub_value = "getfingerprint";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  //--- check the incomming message
    if( strcmp(topic,driversub_topic) == 0){
       if (incommingMessage.equals(driversub_value)){
         ...
         if(stats){
             ...
             for(int i = 0; i < 512; i++){
               ...
               x[i]=fingerTemplate[i];
              }
              
             
             if (!client.connected()) reconnect();
             client.loop();
             client.publish(driverpub_topic,fingerTemplate,512 );
          }else{
